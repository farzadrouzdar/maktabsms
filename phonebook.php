...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_group'])) {
    ...
    $group_name = filter_var($_POST['group_name'], FILTER_SANITIZE_STRING);
    if (!empty($group_name)) {
        $stmt = $pdo->prepare("UPDATE contact_groups SET group_name = ? WHERE id = ? AND school_id = ?");
                    ...
                    <div class="card-body">
                        <form method="POST">
                            ...
                            <?php if ($edit_group): ?>
                                ...
                                <a href="phonebook.php" class="btn btn-secondary">لغو</a>
                            <?php else: ?>
                                ...
                                <?php else: ?>
                                    <?php foreach ($groups as $index => $group): ?>
                                        <tr>
                                            ...
                                            <td><a href="contacts.php?group_id=<?php echo $group['id']; ?>"><?php echo htmlspecialchars($group['group_name']); ?></a></td>
                                            <td><?php echo $group['contact_count']; ?></td>
                                            <td><?php echo date('Y-m-d H:i:s', strtotime($group['created_at'])); ?></td>
                                            <td>
                                                <a href="phonebook.php?edit_group=<?php echo $group['id']; ?>" class="btn btn-sm btn-warning">ویرایش</a>
                                                <a href="phonebook.php?delete_group=<?php echo $group['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('آیا مطمئن هستید که می‌خواهید این گروه را حذف کنید؟');">حذف</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
